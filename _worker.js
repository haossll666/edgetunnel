...
export { 掩码敏感信息, 读取config_JSON as __读取config_JSONForTests };
export default {
...
async function 读取config_JSON(env, hostname, userID, UA = "Mozilla/5.0", 重置配置 = false, 包含管理扩展 = false) {
	...
	const 初始CF用量 = { success: false, pages: 0, workers: 0, total: 0, max: 100000 };
	const 初始化CF_JSON = { Email: null, GlobalAPIKey: null, AccountID: null, APIToken: null, UsageAPI: null };
	const 当前TG配置 = config_JSON.TG && typeof config_JSON.TG === 'object' ? config_JSON.TG : {};
	config_JSON.TG = { 启用: 当前TG配置.启用 === true, ...初始化TG_JSON };
	config_JSON.CF = { ...初始化CF_JSON, Usage: 初始CF用量 };
