				...
				if (区分大小写访问路径 === 加密秘钥) {//快速订阅
					...
					if (url.searchParams.get('token') === 订阅TOKEN || 作为优选订阅生成器) {
						...
						if (订阅类型 === 'mixed') {
							...
							const 稳定首项 = 生成订阅稳定首项(config_JSON);
							...
							订阅内容 = 稳定首项 + 其他节点LINK + 完整优选IP.map(原始地址 => {
								...
								节点备注 = 生成带出口地域的节点备注(节点备注, 节点地址, 节点端口);
...
function 生成订阅稳定首项(config_JSON = {}) {
	if (!config_JSON?.LINK || typeof config_JSON.LINK !== 'string') return '';
	const 稳定首项 = config_JSON.LINK.trim();
	if (!稳定首项) return '';
...
function 生成带出口地域的节点备注(节点备注, 节点地址, 节点端口 = '443') {
	const 原备注 = String(节点备注 || '').trim();
	const 备注 = 原备注 || String(节点地址 || '').trim();
	const 地理信息 = 读取出口地理缓存项(`${String(节点地址 || '').replace(/^\[|\]$/g, '')}:${节点端口}`);
	const 摘要 = 生成出口地域摘要(地理信息);
	if (摘要.status !== 'fresh') return 备注;

	const 地域标签 = [
		摘要.countryCode || 摘要.countryName,
		摘要.city || 摘要.region,
	].map(v => String(v || '').trim()).filter(Boolean).join(' ');

	if (!地域标签 || 备注.startsWith(`${地域标签} | `)) return 备注;
	return `${地域标签} | ${备注}`;
}
