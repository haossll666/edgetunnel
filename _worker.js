		...
		if (访问路径 === 'version' && url.searchParams.get('uuid') === userID) {// 版本信息接口
			...
			if (env.KV && typeof env.KV.get === 'function') {
				...
				if (区分大小写访问路径 === 加密秘钥) {//快速订阅
					...
					config_JSON = await 读取管理配置(env, host, userID, UA);

					if (访问路径 === 'admin/init') {// 重置配置为默认值
						try {
							config_JSON = await 重置并读取管理配置(env, host, userID, UA);
...
async function 读取订阅基础配置(env, hostname, userID, UA = "Mozilla/5.0") {
	return 读取config_JSON(env, hostname, userID, UA, false, false);
}

async function 读取管理配置(env, hostname, userID, UA = "Mozilla/5.0") {
	return 读取config_JSON(env, hostname, userID, UA, false, true);
}
...
async function 读取config_JSON(env, hostname, userID, UA = "Mozilla/5.0", 重置配置 = false, 包含管理扩展 = false) {
	...
	if (!config_JSON.反代 || typeof config_JSON.反代 !== 'object') {
		config_JSON.反代 = {
			[_p]: "auto",
			SOCKS5: {
				启用: 启用SOCKS5反代,
