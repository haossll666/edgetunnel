...
- ✅ **推荐的个人使用流程**：先部署 Worker/Pages，再设置 `ADMIN` 和 `KV`，随后登录 `/admin` 扫码添加订阅。这个二维码 onboarding 是当前最重要的操作路径，默认文档会优先围绕它展开。
- ✅ **默认操作习惯**：如果没有明确需要，先保持 `KEY` 和订阅相关输出不变，优先保证 `/admin` 里扫码加订阅这条主路径稳定可用。
- ✅ **免费层默认值**：个人部署时，优先把 `OFF_LOG` 设为 `1`，把调试日志留在需要排障时再打开，避免不必要的 KV 写入和日志膨胀。
...
4. 访问后台：
   ...
   - 登录后直接使用页面里的二维码，把订阅添加到手机客户端；这是当前推荐且兼容性最稳的订阅 onboarding 方式。
...
| **KEY** | ✅ | `CMLiussss` | Worker 运行所需密钥，同时也是快速订阅路径密钥。个人部署时建议先保持默认值不变，除非你明确要轮换订阅入口，这样最不容易影响 `/admin` 扫码加订阅流程 |
...
> [!TIP]
> 个人部署的默认推荐是：`ADMIN` 负责后台登录，`KEY` 先沿用默认值，`UUID` 只有在你明确想固定节点标识时再设置。这样最容易保住 `/admin` 里的二维码扫码加订阅流程，不会误触发客户端重新添加订阅。

> [!TIP]
> 如果是 Cloudflare 免费层个人部署，先把 `OFF_LOG` 当成节流开关来用：默认建议关闭常驻日志，排障时再临时打开。这样更容易避免 KV 写入压力和日志噪音。
...
> 如果外部 Pages 静态页暂时不可用，Worker 现在会为 `/login`、`/noADMIN`、`/noKV` 提供本地兜底页面，避免把你直接挡在入口外。
> `/admin` 主页面也会在 Pages 不可达时退回到本地 admin shell，至少保住后台入口、配置查看和退出登录这些基本动作。

> [!NOTE]
> 日志策略优先服务于可恢复性，而不是追求完整审计。个人免费层默认建议只保留必要日志，`OFF_LOG` 可以按需打开，避免不必要的 KV 写放大。
> 现在重复的 `Get_SUB` 请求也会先做内存去重，再决定是否写入 KV；这只是在降压，不会改变订阅内容本身。

> [!IMPORTANT]
> 安全优先的个人部署默认不启用远程订阅转换。公共 `SUBAPI` 会收到你的 `/sub?token=...` 订阅入口，并可回源读取 mixed 订阅内容。
> 建议优先使用本地 mixed 订阅；确实需要 Clash、Sing-box、Surge 等转换格式时，请优先使用可信自建转换服务，并显式设置 `REMOTE_SUBCONVERT=1`。

> [!TIP]
> 如果你在中国大陆环境里遇到添加订阅失败，先按这个顺序检查：
> 1. 直接打开 `/admin`，确认后台本身可达。
> 2. 打开 `/admin/config.json`，确认 `config_JSON.LINK` 仍然存在。
> 3. 直接打开 `/admin/diagnostics`，确认恢复入口和订阅摘要还能返回。
> 4. 直接访问 `/sub?token=...`，确认订阅本身返回正常内容。
> 5. 如果浏览器可用但客户端导入失败，优先怀疑客户端网络握手路径，而不是立刻改订阅契约。
> 6. 在不影响现有订阅的前提下，再做日志、节点排序或网络路径的微调。
> 7. 如果是 ChatGPT / X / Claude / Gemini 不稳定，按 [国内 AI 服务可用性 Runbook](docs/RUNBOOK-CN-AI-ACCESS.md) 先区分分流规则、优选 IP 和 PROXYIP 问题。
