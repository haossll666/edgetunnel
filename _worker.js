...
async function 读取config_JSON(env, hostname, userID, UA = "Mozilla/5.0", 重置配置 = false) {
	...
	let 反代查询参数 = '';
	const 反代问号索引 = 路径反代参数.indexOf('?');
	if (反代问号索引 !== -1) {
		反代查询参数 = 路径反代参数.slice(反代问号索引 + 1);
		路径反代参数 = 路径反代参数.slice(0, 反代问号索引);
	}
	...
	let 路径部分;
	let 查询部分;
	const 问号索引 = normalizedPath.indexOf('?');
	if (问号索引 === -1) {
		路径部分 = normalizedPath;
		查询部分 = '';
	} else {
		路径部分 = normalizedPath.slice(0, 问号索引);
		查询部分 = normalizedPath.slice(问号索引);
	}
