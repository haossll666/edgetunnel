...
export { 掩码敏感信息, 是否启用日志记录, 是否允许远程订阅转换, 是否跳过GetSUB日志KV写入, 是否跳过非SUB日志KV写入, 获取Pages页面或本地兜底, 生成本地登录页HTML, 生成本地Admin页HTML, 生成本地NoADMIN页HTML, 生成本地NoKV页HTML, 生成订阅稳定首项, 生成管理诊断视图, 请求日志记录, 读取TG配置, 读取CF配置, 清理配置缓存, 清理基础配置缓存, 清理Cloudflare使用量缓存, 读取config_JSON, 管理员IP绑定模式, 严格模式IP绑定材料, 管理员会话Cookie值, 登录退避_UTC日期键, 登录退避_刷新日计, 登录退避_当日KV写次数, 登录退避_测试置日写次数, 登录退避_计算锁定时长毫秒, 登录退避_测试重置内存, 登录退避_若已锁定则响应, 登录退避_登录成功清理, 登录退避_密码错误响应, 选择反代策略, 清理自动反代池缓存, 清理自动反代健康缓存, 记录自动反代健康结果, 读取自动反代健康分, 设置自动反代策略测试状态, 是否允许记录自动反代健康结果, 过滤自动反代候选, 读取自动反代过滤诊断, 读取自动反代健康摘要, 生成自动反代诊断建议, 生成订阅转换临时Token, 解析地址端口字符串, 解析地址端口 };
export default {
		...
		if (访问路径 === 'version' && url.searchParams.get('uuid') === userID) {// 版本信息接口
			...
			if (env.KV && typeof env.KV.get === 'function') {
				...
				if (区分大小写访问路径 === 加密秘钥) {//快速订阅
					...
					params.set('token', await safeHash(host + userID));
					...
					const [今日订阅转换后端专属TOKEN, 昨日订阅转换后端专属TOKEN] = await Promise.all([
						生成订阅转换临时Token(订阅TOKEN, userID, 0),
						生成订阅转换临时Token(订阅TOKEN, userID, -1),
					]);
					...
					if (用户客户端请求订阅 || 订阅转换后端请求订阅 || 作为优选订阅生成器) {
						...
						if (订阅类型 === 'mixed') {
							...
							const 稳定首项 = 生成订阅稳定首项(config_JSON, !用户客户端请求订阅);
							...
							订阅内容 = 稳定首项 + 其他节点LINK + 完整优选IP.map(原始地址 => {
								...
								if (协议类型 === 'ss' && !作为优选订阅生成器) {
									...
									const alpn参数 = config_JSON.ALPN ? `&alpn=${encodeURIComponent(config_JSON.ALPN)}` : '';
									return `${协议类型}://00000000-0000-4000-8000-000000000000@${节点地址}:${节点端口}?security=tls&type=${传输协议 + ECHLINK参数}&${域名字段名}=example.com&fp=${config_JSON.Fingerprint}&sni=example.com&${路径字段名}=${encodeURIComponent(传输路径参数值) + TLS分片参数}&encryption=none${alpn参数}${config_JSON.跳过证书验证 ? '&insecure=1' : ''}#${encodeURIComponent(节点备注)}`;
								}
							}).filter(item => item !== null).join('\n');
						} else { // 订阅转换
							if (!是否允许远程订阅转换(env, config_JSON)) {
								return new Response('远程订阅转换默认关闭：公共 SUBAPI 会收到你的订阅入口 token。请使用 target=mixed，或设置 REMOTE_SUBCONVERT=1 并配置可信的自建订阅转换服务。', {
									status: 403,
						...
						if (!ua.includes('subconverter') && 用户客户端请求订阅 && !作为优选订阅生成器) 订阅内容 = 批量替换域名(订阅内容.replace(/00000000-0000-4000-8000-000000000000/g, config_JSON.UUID).replace(/MDAwMDAwMDAtMDAwMC00MDAwLTgwMDAtMDAwMDAwMDAwMDAw/g, btoa(config_JSON.UUID)), config_JSON.HOSTS);
...
async function forwardataTCP(host, portNum, rawData, ws, respHeader, remoteConnWrapper, yourUUID) {
	...
	} else if (已配置反代链路() && 是Cloudflare托管站点(host)) {
		log(`[TCP转发] 目标 ${host}:${portNum} 为 Cloudflare 托管站点，优先启用反代链路`);
		try {
			await connecttoPry();
		} catch (err) {
			log(`[TCP转发] Cloudflare 托管站点反代失败: ${err.message}`);
			throw err;
		}
	} else {
		try {
...
function 生成订阅稳定首项(config_JSON = {}, maskUuid = false) {
	if (!config_JSON?.LINK || typeof config_JSON.LINK !== 'string') return '';
	let 稳定首项 = config_JSON.LINK.trim();
	if (!稳定首项) return '';
	if (maskUuid && config_JSON.UUID) {
		稳定首项 = 稳定首项.replace(new RegExp(config_JSON.UUID, 'gi'), '00000000-0000-4000-8000-000000000000')
			.replace(new RegExp(btoa(config_JSON.UUID), 'gi'), 'MDAwMDAwMDAtMDAwMC00MDAwLTgwMDAtMDAwMDAwMDAwMDAw');
	}
...
function 生成管理诊断视图(url, config_JSON = {}, env = {}) {
	...
	return {
		...
		admin: {
			...
			addText: '/admin/ADD.txt',
			cf: '/admin/cf.json',
		},
		subscription: {
			entry: '/sub?token=…',
			stableFirstEntry: Boolean(config_JSON?.LINK),
			updateHours: config_JSON?.优选订阅生成?.SUBUpdateTime || null,
		},
		logging: {
		...
		if (uuid) {
			config.outbounds.forEach(outbound => {
				...
				if ((outbound.uuid && outbound.uuid === uuid) || (outbound.password && outbound.password === uuid)) {
					...
					// 添加/更新 alpn 配置
					if (config_JSON?.ALPN) {
						outbound.tls.alpn = String(config_JSON.ALPN).split(',').map(s => s.trim()).filter(Boolean);
					}
				}
			});
		}
...
async function 请求日志记录(env, request, 访问IP, 请求类型 = "Get_SUB", config_JSON, 是否写入KV日志 = true) {
	try {
		...
		是否写入KV日志 = 是否启用日志记录(env, 是否写入KV日志);
		if (!是否写入KV日志) return;
		if (config_JSON.TG.启用) {
			try {
				const TG_JSON = await 读取TG配置(env);
...
function 是否启用日志记录(env, 是否写入KV日志 = true) {
	return ['1', 'true'].includes(env.OFF_LOG) ? false : 是否写入KV日志;
}

function 是否允许远程订阅转换(env = {}, 配置 = {}) {
	const 明确启用 = (值) => 值 === true || ['1', 'true'].includes(String(值 || '').toLowerCase());
	return 明确启用(env.REMOTE_SUBCONVERT) || 明确启用(配置?.订阅转换配置?.REMOTE);
}

async function 读取TG配置(env, 初始化TG_JSON = {}) {
	try {
		const 当前时间戳 = Date.now();
		const 缓存项 = TG配置缓存.get('tg.json');
		if (缓存项 && 当前时间戳 - 缓存项.TIME < 5 * 60 * 1000) return 缓存项.值;
		const TG_TXT = await env.KV.get('tg.json');
		const TG_JSON = TG_TXT ? JSON.parse(TG_TXT) : 初始化TG_JSON;
		if (!TG_TXT) await env.KV.put('tg.json', JSON.stringify(初始化TG_JSON, null, 2));
		TG配置缓存.set('tg.json', { TIME: 当前时间戳, 值: TG_JSON });
		for (const [key, value] of TG配置缓存.entries()) {
			if (当前时间戳 - value.TIME >= 5 * 60 * 1000) TG配置缓存.delete(key);
		}
		return TG_JSON;
	} catch (error) {
...
async function 读取CF配置(env, 初始化CF_JSON = {}) {
	try {
		const 当前时间戳 = Date.now();
		const 缓存项 = CF配置缓存.get('cf.json');
		if (缓存项 && 当前时间戳 - 缓存项.TIME < 5 * 60 * 1000) return 缓存项.值;
		const CF_TXT = await env.KV.get('cf.json');
		const CF_JSON = CF_TXT ? JSON.parse(CF_TXT) : 初始化CF_JSON;
		if (!CF_TXT) await env.KV.put('cf.json', JSON.stringify(初始化CF_JSON, null, 2));
		CF配置缓存.set('cf.json', { TIME: 当前时间戳, 值: CF_JSON });
		for (const [key, value] of CF配置缓存.entries()) {
			if (当前时间戳 - value.TIME >= 5 * 60 * 1000) CF配置缓存.delete(key);
		}
		return CF_JSON;
	} catch (error) {
...
function 是否跳过非SUB日志KV写入(日志内容) {
	try {
		if (日志内容.TYPE === "Get_SUB") return false;
		const 当前时间戳 = 日志内容.TIME;
		const 签名 = `${日志内容.TYPE}|${日志内容.IP}|${日志内容.URL}|${日志内容.UA}`;
		const 最近写入时间 = 非SUB日志最近写入缓存.get(签名);
		if (最近写入时间 && 当前时间戳 - 最近写入时间 < 30 * 60 * 1000) return true;
		非SUB日志最近写入缓存.set(签名, 当前时间戳);
		for (const [key, value] of 非SUB日志最近写入缓存.entries()) {
			if (当前时间戳 - value >= 30 * 60 * 1000) 非SUB日志最近写入缓存.delete(key);
		}
		return false;
	} catch (error) {
...
async function 生成订阅转换临时Token(订阅TOKEN, userID, 日偏移 = 0) {
	const 日序号 = Math.floor(Date.now() / 86400000) + 日偏移;
	return await safeHash(String(订阅TOKEN) + ':subconv:' + String(userID) + ':' + 日序号);
}
...
async function 读取config_JSON(env, hostname, userID, UA = "Mozilla/5.0", 重置配置 = false, 包含管理扩展 = false) {
	...
	const host = hostname, Ali_DoH = "https://dns.alidns.com/dns-query", ECH_SNI = "cloudflare-ech.com", 占位符 = '{{IP:PORT}}', 初始化开始时间 = performance.now(), 默认配置JSON = {
		...
		优选订阅生成: {
			...
			TOKEN: await safeHash(hostname + userID),
		},
		订阅转换配置: {
	...
	try {
		...
		if (!重置配置 && 缓存项 && 当前时间戳 - 缓存项.TIME < 5 * 60 * 1000) {
			...
			let configJSON = env?.KV ? await env.KV.get('config.json') : null;
			if (!configJSON || 重置配置 == true) {
				if (env?.KV) await env.KV.put('config.json', JSON.stringify(默认配置JSON, null, 2));
				基线配置 = 默认配置JSON;
			} else {
	...
	if (env.HOST) config_JSON.HOSTS = (await 整理成数组(env.HOST)).map(h => { const m = h.match(/^(?:https?:\/\/)?([^/:]+)/i); return (m ? m[1] : h).toLowerCase(); });
	...
	config_JSON.ALPN = (env.ALPN !== undefined && env.ALPN !== null) ? String(env.ALPN).trim() : (config_JSON.ALPN || "");

	if (!config_JSON.反代) config_JSON.反代 = 复制配置(默认配置JSON.反代);
	if (!config_JSON.反代.SOCKS5) config_JSON.反代.SOCKS5 = 复制配置(默认配置JSON.反代.SOCKS5);
	...
	config_JSON.LINK = config_JSON.协议类型 === 'ss'
		...
		: `${config_JSON.协议类型}://${userID}@${host}:443?security=tls&type=${传输协议 + ECHLINK参数}&${域名字段名}=${host}&fp=${config_JSON.Fingerprint}&sni=${host}&${路径字段名}=${encodeURIComponent(传输路径参数值) + TLS分片参数}&encryption=none${alpn参数}${config_JSON.跳过证书验证 ? '&insecure=1' : ''}#${encodeURIComponent(config_JSON.优选订阅生成.SUBNAME)}`;
	config_JSON.优选订阅生成.TOKEN = await safeHash(hostname + userID);
		...
		config_JSON.CF = { ...初始化CF_JSON, Usage: { success: false, pages: 0, workers: 0, total: 0, max: 100000 }, ...(config_JSON.CF || {}) };

		if (包含管理扩展) {
			try {
				const TG_JSON = await 读取TG配置(env, 初始化TG_JSON);
			...
			try {
				const CF_JSON = await 读取CF配置(env, 初始化CF_JSON);
				if (CF_JSON.UsageAPI) {
					try {
...
function 读取自动反代健康摘要() {
	...
	const 每候选最佳分数 = new Map();
	for (const [键, 健康项] of 自动反代健康缓存.entries()) {
		if (健康项.过期时间 <= 当前时间) continue;
		const [候选] = 键.split('@@');
		if (!候选集合.has(候选)) continue;
		const 当前分数 = 读取自动反代健康分(候选, 键.slice(键.indexOf('@@') + 2));
		const 已有分数 = 每候选最佳分数.get(候选);
		if (已有分数 === undefined || 当前分数 > 已有分数) 每候选最佳分数.set(候选, 当前分数);
	}
	let recentHealthyCandidates = 0;
	let recentUnhealthyCandidates = 0;
	let 最高分 = 0;
	for (const 候选 of 候选集合) {
		const 分数 = 每候选最佳分数.get(候选) || 0;
		if (分数 > 0) recentHealthyCandidates++;
		if (分数 < 0) recentUnhealthyCandidates++;
		if (分数 > 最高分) 最高分 = 分数;
	}
	const topScoreBand = 最高分 >= 5 ? 'hot' : (最高分 >= 3 ? 'warm' : (最高分 >= 1 ? 'cool' : 'none'));
	const healthStatus = recentHealthyCandidates > 0
...
globalThis.__edgetunnelUsageCacheForTest = usageCache;
globalThis.__edgetunnelCfUsageCacheForTest = cfUsageCache;

function 清理Cloudflare使用量缓存() {
	usageCache.clear();
	cfUsageCache.clear();
}
...
async function fetchCloudflareUsage(Email, GlobalAPIKey, AccountID, APIToken) {
	...
	// Create a zero-trust cache key out of the credentials
	const cacheKey = `${Email||''}:${GlobalAPIKey||''}:${AccountID||''}:${APIToken||''}`;
	const nowTime = Date.now();
	const TTL = 60000; // 60 seconds

	// Check if we have a valid cache or an in-flight promise
	if (cfUsageCache.has(cacheKey)) {
		const cached = cfUsageCache.get(cacheKey);
		if (nowTime - cached.timestamp < TTL) {
			// If it's a resolved promise with data, return it
			if (cached.data) return cached.data;
			// Otherwise wait for the in-flight promise
			try {
				return await cached.promise;
			} catch (e) {
				// If the cached promise fails, fall through to retry
			}
		}
	}

	// Create the fetching logic inside a promise to cache immediately
	const fetchPromise = (async () => {
		try {
			...
			if (!currentAccountID) {
				...
				currentAccountID = d.result[idx >= 0 ? idx : 0]?.id;
			}
			...
			const res = await fetch(`${API}/graphql`, {
				...
				body: JSON.stringify({
					...
					variables: { AccountID: currentAccountID, filter: { datetime_geq: now.toISOString(), datetime_leq: new Date().toISOString() } }
				})
			});
			...
			const finalData = { success: true, pages, workers, total, max };

			// Store successful data
			const cached = cfUsageCache.get(cacheKey);
			if (cached) cached.data = finalData;
			return finalData;

		} catch (error) {
			console.error('获取使用量错误:', error.message);
			// Remove from cache on error so subsequent requests can retry cleanly
			cfUsageCache.delete(cacheKey);
			return { success: false, pages: 0, workers: 0, total: 0, max: 100000 };
		}
	})();

	// Put in-flight promise into cache
	cfUsageCache.set(cacheKey, { promise: fetchPromise, timestamp: nowTime, data: null });
