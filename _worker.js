...
async function 读取config_JSON(env, hostname, userID, UA = "Mozilla/5.0", 重置配置 = false) {
	...
	const socks5Enabled = config_JSON.反代.SOCKS5.启用;
	const 代理配置 = config_JSON.反代.路径模板[typeof socks5Enabled === 'string' ? socks5Enabled.toUpperCase() : socks5Enabled];

	let 路径反代参数 = '';
	if (代理配置 && config_JSON.反代.SOCKS5.账号) {
		const template = config_JSON.反代.SOCKS5.全局 ? 代理配置.全局 : 代理配置.标准;
		const idx = template.indexOf(占位符);
		路径反代参数 = idx !== -1 ? template.slice(0, idx) + config_JSON.反代.SOCKS5.账号 + template.slice(idx + 占位符.length) : template;
	} else if (config_JSON.反代[_p] !== 'auto') {
		const template = config_JSON.反代.路径模板[_p];
		const idx = template.indexOf(占位符);
		路径反代参数 = idx !== -1 ? template.slice(0, idx) + config_JSON.反代[_p] + template.slice(idx + 占位符.length) : template;
	}

	let 反代查询参数 = '';
	const qIdx = 路径反代参数.indexOf('?');
	if (qIdx !== -1) {
		反代查询参数 = 路径反代参数.slice(qIdx + 1);
		路径反代参数 = 路径反代参数.slice(0, qIdx);
	}

	let path = config_JSON.PATH;
	if (路径反代参数) {
		const paramIdx = path.indexOf(路径反代参数);
		if (paramIdx !== -1) {
			path = path.slice(0, paramIdx) + path.slice(paramIdx + 路径反代参数.length);
		}
	}

	const dSlashIdx = path.indexOf('//');
	if (dSlashIdx !== -1) {
		path = path.slice(0, dSlashIdx) + '/' + path.slice(dSlashIdx + 2);
	}
	...
	if (normalizedPath === '/') {
		...
		const qIdx2 = normalizedPath.indexOf('?');
		if (qIdx2 !== -1) {
			let i = qIdx2 - 1;
			while (i >= 0 && normalizedPath[i] === '/') i--;
			normalizedPath = normalizedPath.slice(0, i + 1) + normalizedPath.slice(qIdx2);
		} else {
			let i = normalizedPath.length - 1;
			while (i >= 0 && normalizedPath[i] === '/') i--;
			normalizedPath = normalizedPath.slice(0, i + 1);
		}
	}

	let 路径部分 = normalizedPath;
	let 查询部分 = '';
	const qIdx3 = normalizedPath.indexOf('?');
	if (qIdx3 !== -1) {
		路径部分 = normalizedPath.slice(0, qIdx3);
		查询部分 = normalizedPath.slice(qIdx3);
	}
