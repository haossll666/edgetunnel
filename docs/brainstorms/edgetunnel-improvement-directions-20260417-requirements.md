...
- **Done（实现）**：连续错误 1–4 次仅内存计数；第 5 次进入锁定时单次 KV put（指数时长，上限 24h）；成功登录 `delete` 清零；isolate 内登录相关 KV 日写 ≥800 时退化为纯内存锁定，UTC 日切恢复计数。
...
- **Done（实现）**：`WorkerGitDescribe` 占位符 + `scripts/stamp-git-describe.mjs`（`git describe --tags --dirty --always`）；`.github/workflows/test.yml` 在测试前 stamp；`/admin/diagnostics` 的 `build.gitDescribe` 与本地 Admin 兜底页展示一致；未改动首行 `Version` 与 `/version?uuid=` 数值契约。
